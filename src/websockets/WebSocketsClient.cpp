...
void WebSocketsClient::begin(const char * host, uint16_t port, const char * url, const char * protocol) {
    ...
    ZZ_DBG_DEBUG("%s", "[WS-Client] Websocket Version: " AX_WEBSOCKETS_VERSION "\n");
}
...
#if defined(HAS_SSL)
        if(_client.isSSL) {
            ZZ_DBG_DEBUG ("[WS-Client] connect wss...\n");
            if(_client.ssl) {
                delete _client.ssl;
                _client.ssl = NULL;
                _client.tcp = NULL;
            ...
            if(_CA_cert) {
                ZZ_DBG_DEBUG("[WS-Client] setting CA certificate");
#if defined(ESP32)
...
#error setCACert not implemented
            ...
            if(_client_cert && _client_key) {
                ...
                AX_DBG_DEBUG("[WS-Client] setting client certificate and key");
#endif
            }
        } else {
            ZZ_DBG_DEBUG("[WS-Client] connect ws...\n");
            if(_client.tcp) {
                delete _client.tcp;
                _client.tcp = NULL;
            }
        ...
        if(!_client.tcp) {
            ZZ_DBG_DEBUG("[WS-Client] creating Network class failed!");
            return;
        }
        WEBSOCKETS_YIELD();
#if defined(ESP32)
...
void WebSocketsClient::clientDisconnect(WSclient_t * client) {
    ...
    ZZ_DBG_DEBUG("[WS-Client] client disconnected.\n");
    if(event) {
...
bool WebSocketsClient::clientIsConnected(WSclient_t * client) {
    ...
    if(client->tcp->connected()) {
        ...
        // client lost
        if(client->status != WSC_NOT_CONNECTED) {
            ZZ_DBG_DEBUG("[WS-Client] connection lost.\n");
            // do cleanup
...
void WebSocketsClient::handleClientData(void) {
    ...
    if((_client.status == WSC_HEADER || _client.status == WSC_BODY) && _lastHeaderSent + WEBSOCKETS_TCP_TIMEOUT < millis()) {
        ZZ_DBG_DEBUG("[WS-Client][handleClientData] header response timeout.. disconnecting!\n");
...
void WebSocketsClient::sendHeader(WSclient_t * client) {
    ...
    ZZ_DBG_DEBUG("[WS-Client][sendHeader] sending header...\n");
    ...
    ZZ_DBG_DEBUG("[WS-Client][sendHeader] handshake %s", (uint8_t *)handshake.c_str());
    ...
    ZZ_DBG_DEBUG("[WS-Client][sendHeader] sending header... Done (%luus).\n", (micros() - start));
...
void WebSocketsClient::handleHeader(WSclient_t * client, String * headerLine) {
    ...
    if(headerLine->length() > 0 && client->isSocketIO && client->status == WSC_BODY && client->cSessionId.length() == 0) {
        ZZ_DBG_DEBUG("[WS-Client][handleHeader] socket.io json: %s\n", headerLine->c_str());
        ...
        if(headerLine->indexOf(sid_begin) > -1) {
            ...
            ZZ_DBG_DEBUG("[WS-Client][handleHeader]  - cSessionId: %s\n", client->cSessionId.c_str());
    ...
    if(headerLine->length() > 0) {
        ZZ_DBG_DEBUG("[WS-Client][handleHeader] RX: %s\n", headerLine->c_str());
        ...
        if(headerLine->startsWith(WEBSOCKETS_STRING("HTTP/1."))) {
            ...
            ZZ_DBG_DEBUG("[WS-Client][handleHeader] Header error (%s)\n", headerLine->c_str());
        }
        ...
        ZZ_DBG_DEBUG("[WS-Client][handleHeader] Header read fin.\n");
        ZZ_DBG_DEBUG("[WS-Client][handleHeader] Client settings:\n");

        ZZ_DBG_DEBUG("[WS-Client][handleHeader]  - cURL: %s\n", client->cUrl.c_str());
        ZZ_DBG_DEBUG("[WS-Client][handleHeader]  - cKey: %s\n", client->cKey.c_str());

        ZZ_DBG_DEBUG("[WS-Client][handleHeader] Server header:\n");
        ZZ_DBG_DEBUG("[WS-Client][handleHeader]  - cCode: %d\n", client->cCode);
        ZZ_DBG_DEBUG("[WS-Client][handleHeader]  - cIsUpgrade: %d\n", client->cIsUpgrade);
        ZZ_DBG_DEBUG("[WS-Client][handleHeader]  - cIsWebsocket: %d\n", client->cIsWebsocket);
        ZZ_DBG_DEBUG("[WS-Client][handleHeader]  - cAccept: %s\n", client->cAccept.c_str());
        ZZ_DBG_DEBUG("[WS-Client][handleHeader]  - cProtocol: %s\n", client->cProtocol.c_str());
        ZZ_DBG_DEBUG("[WS-Client][handleHeader]  - cExtensions: %s\n", client->cExtensions.c_str());
        ZZ_DBG_DEBUG("[WS-Client][handleHeader]  - cVersion: %d\n", client->cVersion);
        ZZ_DBG_DEBUG("[WS-Client][handleHeader]  - cSessionId: %s\n", client->cSessionId.c_str());

        if(client->isSocketIO && client->cSessionId.length() == 0 && clientIsConnected(client)) {
            ZZ_DBG_DEBUG("[WS-Client][handleHeader] still missing cSessionId try socket.io V3\n");
        ...
        if(ok) {
            switch(client->cCode) {
                ...
                default:     ///< Server dont unterstand requrst
                    ok = false;
                    ZZ_DBG_DEBUG("[WS-Client][handleHeader] serverCode is not 101 (%d)\n", client->cCode);
                    clientDisconnect(client);
                    _lastConnectionFail = millis();
                    break;
            }
        ...
        if(ok) {
            if(client->cAccept.length() == 0) {
                ...
                if(sKey != client->cAccept) {
                    ZZ_DBG_DEBUG("[WS-Client][handleHeader] Sec-WebSocket-Accept is wrong\n");
                    ok = false;
                }
            }
        }
        ...
        if(ok) {
            ZZ_DBG_DEBUG("[WS-Client][handleHeader] Websocket connection init done.\n");
            headerDone(client);
            ...
            if(client->cSessionId.length() > 0) {
                ZZ_DBG_DEBUG("[WS-Client][handleHeader] found cSessionId\n");
                if(clientIsConnected(client) && _client.tcp->available()) {
                    // read not needed data
                    ZZ_DBG_DEBUG("[WS-Client][handleHeader] still data in buffer (%d), clean up.\n", _client.tcp->available());
            ...
            ZZ_DBG_DEBUG("[WS-Client][handleHeader] no Websocket connection close.\n");
            _lastConnectionFail = millis();
...
void WebSocketsClient::connectedCb() {
    ...
    if(_client.isSSL && SSL_FINGERPRINT_IS_SET) {
        if(!_client.ssl->verify(_fingerprint.c_str(), _host.c_str())) {
            ZZ_DBG_DEBUG("[WS-Client] certificate mismatch\n");
            WebSockets::clientDisconnect(&_client, 1000);
            return;
        }
#else
...
void WebSocketsClient::connectFailedCb() {
    ZZ_DBG_DEBUG("[WS-Client] connection to %s:%u Failed\n", _host.c_str(), _port);
}

#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)

void WebSocketsClient::asyncConnect() {
    AX_DBG_DEBUG("[WS-Client] asyncConnect...\n");

    AsyncClient * tcpclient = new AsyncClient();

    if(!tcpclient) {
        AX_DBG_DEBUG("[WS-Client] creating AsyncClient class failed!\n");
        return;
    }
    ...
    tcpclient->onConnect(std::bind([](WebSocketsClient * ws, AsyncClient * tcp) {
        ...
        if(!ws->_client.tcp) {
            AX_DBG_DEBUG("[WS-Client] creating Network class failed!\n");
            ws->connectFailedCb();
            return;
        }
        ws->connectedCb();
...
void WebSocketsClient::handleHBPing() {
    ...
    if(pi > _client.pingInterval) {
        ZZ_DBG_DEBUG("[WS-Client] sending HB ping\n");
        if(sendPing()) {
            _client.lastPing     = millis();
            _client.pongReceived = false;
        } else {
            ZZ_DBG_DEBUG("[WS-Client] sending HB ping failed\n");
            WebSockets::clientDisconnect(&_client, 1000);
        }
    }
}

void WebSocketsClient::handleHeaderLine()
{
  char buffer_sz[ 1024 ];
  String strData;
  int ind = 0;
  ...
  _client.tcp->readBytes( buffer_sz, 1024 );
   strData = String( buffer_sz );
  while( 1 )
  {
    ind1 = strData.indexOf( '\n', ind );
    if( ind1 >= 0 ) {

      String headerLine = strData.substring( ind, ind1 );
      handleHeader( &_client, &headerLine );
      ind  = ind1 + 1;
      delay( 10 );
    }
