...
void WebSockets::clientDisconnect(WSclient_t * client, uint16_t code, char * reason, size_t reasonLen)
{
  ...
  ZZ_DBG_DEBUG ("[WS][%d][handleWebsocket] clientDisconnect code: %u\n", client->num, code);
...
bool WebSockets::sendFrame(WSclient_t * client, WSopcode_t opcode, uint8_t * payload, size_t length, bool fin, bool headerToPayload) {
#if (WEBSOCKETS_NETWORK_TYPE == NETWORK_SERIAL)
    if(client->tcp->hasValidCommandTime() && !client->tcp->isTimeOut())
    {
        ZZ_DBG_INFO ("[WS][%d][sendFrame] Waiting for the reply...!\n", client->num);
        return false;
    }
#endif /* NETWORK_SERIAL */
    if(client->tcp && !client->tcp->connected()) {
        ZZ_DBG_DEBUG ("[WS][%d][sendFrame] not Connected!?\n", client->num);
        return false;
    }

    if(client->status != WSC_CONNECTED) {
        ZZ_DBG_DEBUG ("[WS][%d][sendFrame] not in WSC_CONNECTED state!?\n", client->num);
        return false;
    }

    ZZ_DBG_DEBUG ("[WS][%d][sendFrame] ------- send message frame -------\n", client->num);
    ZZ_DBG_DEBUG ("[WS][%d][sendFrame] fin: %u opCode: %u mask: %u length: %u headerToPayload: %u\n", client->num, fin, opcode, client->cIsClient, length, headerToPayload);

    if(opcode == WSop_text) {
        ZZ_DBG_DEBUG ("[WS][%d][sendFrame] text: %s\n", client->num, (payload + (headerToPayload ? 14 : 0)));
    }
    ...
    if(!headerToPayload && ((length > 0) && (length < 1400)) && (GET_FREE_HEAP > 6000)) {
        ZZ_DBG_DEBUG("[WS][%d][sendFrame] pack to one TCP package...\n", client->num);
    ...
    ZZ_DBG_DEBUG ("[WS][%d][sendFrame] sending Frame Done (%luus).\n", client->num, (micros() - start));
...
void WebSockets::headerDone(WSclient_t * client) {
    ...
    client->cWsRXsize = 0;
    ZZ_DBG_INFO ("[WS][%d][headerDone] Header Handling Done.\n", client->num);
...
bool WebSockets::handleWebsocketWaitFor(WSclient_t * client, size_t size) {
    ...
    if(size > WEBSOCKETS_MAX_HEADER_SIZE) {
        ZZ_DBG_DEBUG ("[WS][%d][handleWebsocketWaitFor] size: %d too big!\n", client->num, size);
        return false;
    }
    ...
    ZZ_DBG_DEBUG ("[WS][%d][handleWebsocketWaitFor] size: %d cWsRXsize: %d\n", client->num, size, client->cWsRXsize);
    readCb(client, &client->cWsHeader[client->cWsRXsize], (size - client->cWsRXsize), std::bind([](WebSockets * server, size_t size, WSclient_t * client, bool ok) {
        ZZ_DBG_DEBUG("[WS][%d][handleWebsocketWaitFor][readCb] size: %d ok: %d\n", client->num, size, ok);
        if(ok) {
            client->cWsRXsize = size;
            server->handleWebsocketCb(client);
        } else {
            ZZ_DBG_DEBUG("[WS][%d][readCb] failed.\n", client->num);
            client->cWsRXsize = 0;
            // timeout or error
...
void WebSockets::handleWebsocketCb(WSclient_t * client) {
    ...
    ZZ_DBG_DEBUG("[WS][%d][handleWebsocket] ------- read massage frame -------\n", client->num);
    ZZ_DBG_DEBUG("[WS][%d][handleWebsocket] fin: %u rsv1: %u rsv2: %u rsv3 %u  opCode: %u\n", client->num, header->fin, header->rsv1, header->rsv2, header->rsv3, header->opCode);
    ZZ_DBG_DEBUG("[WS][%d][handleWebsocket] mask: %u payloadLen: %u\n", client->num, header->mask, header->payloadLen);

    if(header->payloadLen > WEBSOCKETS_MAX_DATA_SIZE) {
        ZZ_DBG_DEBUG("[WS][%d][handleWebsocket] payload too big! (%u)\n", client->num, header->payloadLen);
    ...
    if(header->payloadLen > 0) {
        ...
        if(!payload) {
            ZZ_DBG_DEBUG("[WS][%d][handleWebsocket] to less memory to handle payload %d!\n", client->num, header->payloadLen);
...
void WebSockets::handleWebsocketPayloadCb(WSclient_t * client, bool ok, uint8_t * payload) {
    ...
    if(ok) {
        ...
        switch(header->opCode) {
            case WSop_text:
                ZZ_DBG_DEBUG("[WS][%d][handleWebsocket] text: %s\n", client->num, payload);
                // no break here!
            case WSop_binary:
            ...
            case WSop_ping:
                // send pong back
                ZZ_DBG_DEBUG("[WS][%d][handleWebsocket] ping received (%s)\n", client->num, payload ? (const char *)payload : "");
                sendFrame(client, WSop_pong, payload, header->payloadLen);
                messageReceived(client, header->opCode, payload, header->payloadLen, header->fin);
                break;
            case WSop_pong:
                ZZ_DBG_DEBUG("[WS][%d][handleWebsocket] get pong (%s)\n", client->num, payload ? (const char *)payload : "");
...
#endif
                ZZ_DBG_DEBUG("[WS][%d][handleWebsocket] get ask for close. Code: %d\n", client->num, reasonCode);
                if(header->payloadLen > 2) {
                    ZZ_DBG_DEBUG(" (%s)\n", (payload + 2));
                } else {
                    ZZ_DBG_DEBUG("\n");
            ...
            default:
                ZZ_DBG_DEBUG("[WS][%d][handleWebsocket] got unknown opcode: %d\n", client->num, header->opCode);
        ...
        ZZ_DBG_DEBUG("[WS][%d][handleWebsocket] missing data!\n", client->num);
        free(payload);
        clientDisconnect(client, 1002);
    }
}
...
bool WebSockets::readCb(WSclient_t * client, uint8_t * out, size_t n, WSreadWaitCb cb) {
    ...
    ssize_t len;
    ZZ_DBG_DEBUG("[readCb] n: %zu t: %lu\n", n, t);
    while(n > 0) {
        if(client->tcp == NULL) {
            ZZ_DBG_DEBUG("[readCb] tcp is null!\n");
            if(cb) {
                cb(client, false);
            }
            return false;
        ...
        if(!client->tcp->connected()) {
            ZZ_DBG_DEBUG("[readCb] not connected!\n");
            if(cb) {
                cb(client, false);
            }
            return false;
        ...
        if((millis() - t) > WEBSOCKETS_TCP_TIMEOUT) {
            ZZ_DBG_DEBUG("[readCb] receive TIMEOUT! %lu\n", (millis() - t));
            if(cb) {
                cb(client, false);
            }
            return false;
...
#if (WEBSOCKETS_NETWORK_TYPE == NETWORK_SERIAL)
        Serial.printf( "[tcp-read]: %d %d -> ", n, len );
        for( int i = 0; i < n; i++ )
        {
...
size_t WebSockets::write(WSclient_t * client, uint8_t * out, size_t n) {
    ...
    size_t len      = 0;
    size_t total    = 0;
    ZZ_DBG_DEBUG("[write] n: %zu t: %lu\n", n, t);
    while(n > 0) {
        if(client->tcp == NULL) {
            ZZ_DBG_DEBUG("[write] tcp is null!\n");
            break;
        }

        if(!client->tcp->connected()) {
            ZZ_DBG_DEBUG("[write] not connected!\n");
            break;
        }

        if((millis() - t) > WEBSOCKETS_TCP_TIMEOUT) {
            ZZ_DBG_DEBUG("[write] write TIMEOUT! %lu\n", (millis() - t));
            break;
        }
#if (WEBSOCKETS_NETWORK_TYPE == NETWORK_SERIAL)
        Serial.printf( "[tcp-write]: %d -> ", n );
        for( int i = 0; i < n; i++ )
        {
          Serial.printf( "%X ", out[i] );
        }
        ...
        if(len) {
            ...
            n -= len;
            total += len;
            // ZZ_DBG_DEBUG("write %d left %d!\n", len, n);
        } else {
            ZZ_DBG_DEBUG("WS write %d failed left %d!\n", len, n);
        }
        if(n > 0) {
            WEBSOCKETS_YIELD();
        }
