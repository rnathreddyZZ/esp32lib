...
static void _handle_async_event(lwip_event_packet_t * e){
    if(e->event == LWIP_TCP_CLEAR){
        ...
        //ets_printf("-R: 0x%08x\n", e->recv.pcb);
        ZZAsyncClient::_s_recv(e->arg, e->recv.pcb, e->recv.pb, e->recv.err);
    } else if(e->event == LWIP_TCP_FIN){
        //ets_printf("-F: 0x%08x\n", e->fin.pcb);
        ZZAsyncClient::_s_fin(e->arg, e->fin.pcb, e->fin.err);
    } else if(e->event == LWIP_TCP_SENT){
        //ets_printf("-S: 0x%08x\n", e->sent.pcb);
        ZZAsyncClient::_s_sent(e->arg, e->sent.pcb, e->sent.len);
    } else if(e->event == LWIP_TCP_POLL){
        //ets_printf("-P: 0x%08x\n", e->poll.pcb);
        ZZAsyncClient::_s_poll(e->arg, e->poll.pcb);
    } else if(e->event == LWIP_TCP_ERROR){
        //ets_printf("-E: 0x%08x %d\n", e->arg, e->error.err);
        ZZAsyncClient::_s_error(e->arg, e->error.err);
    } else if(e->event == LWIP_TCP_CONNECTED){
        //ets_printf("C: 0x%08x 0x%08x %d\n", e->arg, e->connected.pcb, e->connected.err);
        ZZAsyncClient::_s_connected(e->arg, e->connected.pcb, e->connected.err);
    } else if(e->event == LWIP_TCP_ACCEPT){
        //ets_printf("A: 0x%08x 0x%08x\n", e->arg, e->accept.client);
        ZZAsyncServer::_s_accepted(e->arg, e->accept.client);
    } else if(e->event == LWIP_TCP_DNS){
        //ets_printf("D: 0x%08x %s = %s\n", e->arg, e->dns.name, ipaddr_ntoa(&e->dns.addr));
        ZZAsyncClient::_s_dns_found(e->dns.name, &e->dns.addr, e->arg);
    }
    free((void*)(e));
}
...
static int8_t _tcp_recv(void * arg, struct tcp_pcb * pcb, struct pbuf *pb, int8_t err) {
    ...
    if(pb){
        ...
        ZZAsyncClient::_s_lwip_fin(e->arg, e->fin.pcb, e->fin.err);
    }
    if (!_send_async_event(&e)) {
        free((void*)(e));
    }
...
static int8_t _tcp_accept(void * arg, ZZAsyncClient * client) {
    lwip_event_packet_t * e = (lwip_event_packet_t *)malloc(sizeof(lwip_event_packet_t));
...
ZZAsyncClient::ZZAsyncClient(tcp_pcb* pcb)
: _connect_cb(0)
, _connect_cb_arg(0)
, _discard_cb(0)
, _discard_cb_arg(0)
...
ZZAsyncClient& ZZAsyncClient::operator=(const ZZAsyncClient& other){
    if (_pcb) {
        _close();
    }
...
bool ZZAsyncClient::operator==(const ZZAsyncClient &other) {
    return _pcb == other._pcb;
}

ZZAsyncClient & ZZAsyncClient::operator+=(const ZZAsyncClient &other) {
    if(next == NULL){
        next = (ZZAsyncClient*)(&other);
        next->prev = this;
    } else {
        ZZAsyncClient *c = next;
        while(c->next != NULL) {
            c = c->next;
        }
        c->next =(ZZAsyncClient*)(&other);
        c->next->prev = c;
    }
    return *this;
}
...
void ZZAsyncClient::onConnect(AcConnectHandler cb, void* arg){
    _connect_cb = cb;
    _connect_cb_arg = arg;
}

void ZZAsyncClient::onDisconnect(AcConnectHandler cb, void* arg){
    _discard_cb = cb;
    _discard_cb_arg = arg;
}

void ZZAsyncClient::onAck(AcAckHandler cb, void* arg){
    _sent_cb = cb;
    _sent_cb_arg = arg;
}

void ZZAsyncClient::onError(AcErrorHandler cb, void* arg){
    _error_cb = cb;
    _error_cb_arg = arg;
}

void ZZAsyncClient::onData(AcDataHandler cb, void* arg){
    _recv_cb = cb;
    _recv_cb_arg = arg;
}

void ZZAsyncClient::onPacket(AcPacketHandler cb, void* arg){
  _pb_cb = cb;
  _pb_cb_arg = arg;
}

void ZZAsyncClient::onTimeout(AcTimeoutHandler cb, void* arg){
    _timeout_cb = cb;
    _timeout_cb_arg = arg;
}

void ZZAsyncClient::onPoll(AcConnectHandler cb, void* arg){
    _poll_cb = cb;
    _poll_cb_arg = arg;
}
...
bool ZZAsyncClient::connect(IPAddress ip, uint16_t port){
    if (_pcb){
...
bool ZZAsyncClient::connect(const char* host, uint16_t port){
    ip_addr_t addr;
...
void ZZAsyncClient::close(bool now){
    if(_pcb){
...
size_t ZZAsyncClient::add(const char* data, size_t size, uint8_t apiflags) {
...
size_t ZZAsyncClient::ack(size_t len){
    if(len > _rx_ack_len)
        len = _rx_ack_len;
    if(len){
...
void ZZAsyncClient::ackPacket(struct pbuf * pb){
  if(!pb){
    return;
  }
...
int8_t ZZAsyncClient::_connected(void* pcb, int8_t err){
...
void ZZAsyncClient::_error(int8_t err) {
    if(_pcb){
        tcp_arg(_pcb, NULL);
        tcp_sent(_pcb, NULL);
        tcp_recv(_pcb, NULL);
...
//In LwIP Thread
int8_t ZZAsyncClient::_lwip_fin(tcp_pcb* pcb, int8_t err) {
    if(!_pcb || pcb != _pcb){
...
//In Async Thread
int8_t ZZAsyncClient::_fin(tcp_pcb* pcb, int8_t err) {
    _tcp_clear_events(this);
    if(_discard_cb) {
        _discard_cb(_discard_cb_arg, this);
    }
...
int8_t ZZAsyncClient::_sent(tcp_pcb* pcb, uint16_t len) {
    _rx_last_packet = millis();
    //log_i("%u", len);
    _pcb_busy = false;
    if(_sent_cb) {
...
int8_t ZZAsyncClient::_recv(tcp_pcb* pcb, pbuf* pb, int8_t err) {
    while(pb != NULL) {
        _rx_last_packet = millis();
...
int8_t ZZAsyncClient::_poll(tcp_pcb* pcb){
    if(!_pcb){
        log_w("pcb is NULL");
        return ERR_OK;
    }
...
void ZZAsyncClient::_dns_found(struct ip_addr *ipaddr){
...
void ZZAsyncClient::stop() {
    close(false);
}

bool ZZAsyncClient::free(){
    if(!_pcb) {
        return true;
    }
...
size_t ZZAsyncClient::write(const char* data) {
    if(data == NULL) {
        return 0;
    }
...
size_t ZZAsyncClient::write(const char* data, size_t size, uint8_t apiflags) {
...
void ZZAsyncClient::setRxTimeout(uint32_t timeout){
    _rx_since_timeout = timeout;
}

uint32_t ZZAsyncClient::getRxTimeout(){
    return _rx_since_timeout;
}

uint32_t ZZAsyncClient::getAckTimeout(){
    return _ack_timeout;
}

void ZZAsyncClient::setAckTimeout(uint32_t timeout){
    _ack_timeout = timeout;
}

void ZZAsyncClient::setNoDelay(bool nodelay){
    if(!_pcb) {
        return;
    }
    if(nodelay) {
...
uint32_t ZZAsyncClient::getRemoteAddress() {
    if(!_pcb) {
        return 0;
    }
...
uint16_t ZZAsyncClient::getRemotePort() {
    if(!_pcb) {
        return 0;
    }
...
uint32_t ZZAsyncClient::getLocalAddress() {
    if(!_pcb) {
        return 0;
    }
...
uint16_t ZZAsyncClient::getLocalPort() {
    if(!_pcb) {
        return 0;
    }
...
IPAddress ZZAsyncClient::remoteIP() {
    return IPAddress(getRemoteAddress());
}

uint16_t ZZAsyncClient::remotePort() {
    return getRemotePort();
}

IPAddress ZZAsyncClient::localIP() {
    return IPAddress(getLocalAddress());
}

uint16_t ZZAsyncClient::localPort() {
    return getLocalPort();
}

uint8_t ZZAsyncClient::state() {
    if(!_pcb) {
        return 0;
    }
    return _pcb->state;
...
bool ZZAsyncClient::disconnecting(){
    if (!_pcb) {
        return false;
    }
...
bool ZZAsyncClient::disconnected(){
    if (!_pcb) {
        return true;
    }
...
bool ZZAsyncClient::canSend(){
    return space() > 0;
}

const char * ZZAsyncClient::errorToString(int8_t error){
    switch(error){
...
const char * ZZAsyncClient::stateToString(){
    switch(state()){
...
void ZZAsyncClient::_s_dns_found(const char * name, struct ip_addr * ipaddr, void * arg){
    reinterpret_cast<ZZAsyncClient*>(arg)->_dns_found(ipaddr);
}

int8_t ZZAsyncClient::_s_poll(void * arg, struct tcp_pcb * pcb) {
    return reinterpret_cast<ZZAsyncClient*>(arg)->_poll(pcb);
}

int8_t ZZAsyncClient::_s_recv(void * arg, struct tcp_pcb * pcb, struct pbuf *pb, int8_t err) {
    return reinterpret_cast<ZZAsyncClient*>(arg)->_recv(pcb, pb, err);
}

int8_t ZZAsyncClient::_s_fin(void * arg, struct tcp_pcb * pcb, int8_t err) {
    return reinterpret_cast<ZZAsyncClient*>(arg)->_fin(pcb, err);
}

int8_t ZZAsyncClient::_s_lwip_fin(void * arg, struct tcp_pcb * pcb, int8_t err) {
    return reinterpret_cast<ZZAsyncClient*>(arg)->_lwip_fin(pcb, err);
}

int8_t ZZAsyncClient::_s_sent(void * arg, struct tcp_pcb * pcb, uint16_t len) {
    return reinterpret_cast<ZZAsyncClient*>(arg)->_sent(pcb, len);
}

void ZZAsyncClient::_s_error(void * arg, int8_t err) {
    reinterpret_cast<ZZAsyncClient*>(arg)->_error(err);
}

int8_t ZZAsyncClient::_s_connected(void * arg, void * pcb, int8_t err){
    return reinterpret_cast<ZZAsyncClient*>(arg)->_connected(pcb, err);
}
...
ZZAsyncServer::ZZAsyncServer(IPAddress addr, uint16_t port)
: _port(port)
, _addr(addr)
, _noDelay(false)
, _pcb(0)
...
ZZAsyncServer::ZZAsyncServer(uint16_t port)
: _port(port)
...
ZZAsyncServer::~ZZAsyncServer(){
    end();
}

void ZZAsyncServer::onClient(AcConnectHandler cb, void* arg){
    _connect_cb = cb;
    _connect_cb_arg = arg;
}

void ZZAsyncServer::begin(){
    if(_pcb) {
        return;
    }
...
//runs on LwIP thread
int8_t ZZAsyncServer::_accept(tcp_pcb* pcb, int8_t err){
    //ets_printf("+A: 0x%08x\n", pcb);
    if(_connect_cb){
        ZZAsyncClient *c = new ZZAsyncClient(pcb);
        if(c){
...
int8_t ZZAsyncServer::_accepted(ZZAsyncClient* client){
    if(_connect_cb){
...
void ZZAsyncServer::setNoDelay(bool nodelay){
    _noDelay = nodelay;
}

bool ZZAsyncServer::getNoDelay(){
    return _noDelay;
}

uint8_t ZZAsyncServer::status(){
    if (!_pcb) {
        return 0;
    }
    return _pcb->state;
...
int8_t ZZAsyncServer::_s_accept(void * arg, tcp_pcb * pcb, int8_t err){
    return reinterpret_cast<ZZAsyncServer*>(arg)->_accept(pcb, err);
}

int8_t ZZAsyncServer::_s_accepted(void *arg, ZZAsyncClient* client){
    return reinterpret_cast<ZZAsyncServer*>(arg)->_accepted(client);
}
